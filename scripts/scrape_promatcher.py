#!/usr/bin/env python3
"""
Scrape ProMatcher plumbing pricing estimates into competitor_pricing with embeddings.
"""
...
PROMATCHER_URLS = [
    "https://plumbers.promatcher.com/cost/",
    "https://plumbers.promatcher.com/cost/plumbing-repair-costs-prices.aspx",
    "https://plumbers.promatcher.com/cost/water-heater-installation-costs-prices.aspx",
    "https://plumbers.promatcher.com/cost/drain-cleaning-costs-prices.aspx",
    "https://plumbers.promatcher.com/cost/toilet-repair-costs-prices.aspx",
    "https://plumbers.promatcher.com/cost/faucet-repair-costs-prices.aspx",
    "https://plumbers.promatcher.com/cost/garbage-disposal-costs-prices.aspx",
    "https://plumbers.promatcher.com/cost/sewer-line-repair-costs-prices.aspx",
    "https://plumbers.promatcher.com/cost/gas-line-installation-costs-prices.aspx",
    "https://plumbers.promatcher.com/cost/gas-line-repair-costs-prices.aspx",
    "https://plumbers.promatcher.com/cost/repiping-costs-prices.aspx",
    "https://plumbers.promatcher.com/cost/sump-pump-costs-prices.aspx",
]
...
def parse_with_claude(anthropic_api_key: str, markdown: str, source_url: str) -> list[PricingRow]:
    prompt = f"""Extract plumbing service pricing estimates from this ProMatcher page.
...
Rules:
- Extract plumbing pricing entries only.
- If one value appears, set low=avg=high.
- Keep notes concise with source context.
...
def upsert_rows(supabase_url: str, service_role_key: str, rows: list[dict[str, Any]]) -> None:
    ...
    requests.delete(
        f"{supabase_url}/rest/v1/competitor_pricing?source=eq.promatcher&region=eq.US_DFW_REFERENCE",
        headers=headers,
        timeout=60,
    ).raise_for_status()
    response = requests.post(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Scrape ProMatcher pricing into competitor_pricing")
    ...
    urls = args.url if args.url else PROMATCHER_URLS
    ...
    for url in urls:
        try:
            ...
            for row in parsed_rows:
                emb_text = (
                    f"source=promatcher; service_type={row.service_type}; unit={row.unit}; "
